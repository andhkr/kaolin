...
# import kaolin.physics.simplicits.dual_quat as dual_quat_nlbs
...
def weight_function_lbs(x0, tfms, fcn):
    ...
    """
    w_x0 = fcn(x0)
    # w_sum = torch.sum(w_x0, dim=1, keepdim=True)
    # w_x0_normalized = w_x0 / torch.clamp(w_sum, min=1e-9)

    # if method=='lbs':
    return standard_lbs(x0, tfms, w_x0)
    #     return dual_quat_nlbs.dual_quaternion_nlbs(x0,tfms,w_x0_normalized)
    # elif method == 'dual_quat':
    #     return dual_quaternion_skinning(x0, tfms, w_x0)
    # else:
    #     print('called')
    #     return dual_quat_nlbs.dual_quaternion_nlbs(x0,tfms,w_x0)


def apply_skinning(x0,tfms,fcn, skinning_impl_func):
     """Applies chosen skinning implementation with normalized weights."""
     w_x0 = fcn(x0)
    #  # Ensure weights sum to 1
    #  w_sum = torch.sum(w_x0, dim=1, keepdim=True)
    #  # Add small epsilon before normalization to avoid NaN gradients if sum is zero
    #  w_x0_normalized = w_x0 / torch.clamp(w_sum, min=1e-9)
     return skinning_impl_func(x0, tfms, w_x0)
